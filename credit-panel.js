...
function renderExecutive(){
  ...
  const {digits,pos,sig}=filtered(), validSig=sig.filter(eligibleSignature);
  const saldoDigitos=digits.reduce((a,r)=>a+Number(r.saldo||0),0), assinatura=validSig.reduce((a,r)=>a+Number(r.valorUsd||0),0), credito=saldoDigitos+assinatura;
  const empenhado=pos.reduce((a,r)=>a+Number(r.valorUsd||0),0), total=credito+empenhado;
  ...
  $('#kpiCreditAvailable').textContent=money(credito); $('#kpiCommitted').textContent=money(empenhado); $('#kpiSigning').textContent=money(assinatura); $('#kpiReceived').textContent=money(total); $('#kpiAvailablePct').textContent=pct(credito,total); $('#kpiPoCount')&&($('#kpiPoCount').textContent=num(pos.length)); $('#kpiSuppliersCount')&&($('#kpiSuppliersCount').textContent=num(fornecedores.length)); $('#kpiPoBilled')&&($('#kpiPoBilled').textContent=money(faturadoPo)); $('#kpiPoBalance')&&($('#kpiPoBalance').textContent=money(saldoPo));
  const table=$('#executiveSummaryBody'); if(table)table.innerHTML=`<tr><td>2026</td><td class="text-right">${money(credito)}</td><td class="text-right">${money(empenhado)}</td><td class="text-right">${money(assinatura)}</td><td class="text-right">${money(total)}</td><td class="text-center">${pct(credito,total)}</td></tr>`;
  drawBar('creditByGcChart',agg(digits.concat(validSig),gcFromDigit,r=>r.saldo!==undefined?r.saldo:r.valorUsd),'Crédito disponível por Grande Comando','Saldo + POs em assinatura');
  drawBar('poByGcChart',agg(pos,gcFromDigit,r=>r.valorUsd),'PO por Grande Comando conforme dígito','Valor empenhado'); drawBar('poByCompanyChart',agg(pos,r=>r.fornecedor||'N/I',r=>r.valorUsd),'Empenhos por empresa','Valor empenhado'); drawBar('poByProjectChart',agg(pos,r=>projectLabels(r).join(', ')||'Sem projeto',r=>r.valorUsd),'Empenhos por projeto','Valor empenhado'); renderDigitsTable(digits);
}
...
async function generateCreditReport(){
  const {digits,pos,sig}=filtered(), validSig=sig.filter(eligibleSignature);
  const saldoDigitos=digits.reduce((a,r)=>a+Number(r.saldo||0),0), assinatura=validSig.reduce((a,r)=>a+Number(r.valorUsd||0),0), credito=saldoDigitos+assinatura, empenhado=pos.reduce((a,r)=>a+Number(r.valorUsd||0),0), total=credito+empenhado;
