...
function rowMatches(r,f){return (!f.ug||r.ugr===f.ug||r.sigla===f.ug)&&(!f.acao||r.acao===f.acao)&&(!f.natureza||r.natureza===f.natureza)&&matchesProject(r,f.projeto)}
function filtered(){ const f=valuesOfFilter(); return {digits:data.digits.filter(r=>rowMatches(r,f)), pos:data.purchaseOrders.filter(r=>rowMatches(r,f)), sig:data.signatureOrders.filter(r=>rowMatches(r,f)), f}; }
...
function renderUG(){
  ...
  const {digits}=filtered(); const map=new Map(); digits.forEach(r=>{const k=r.sigla||r.ugr||'N/I'; if(!map.has(k)) map.set(k,{sigla:k,nome:r.nomeUgr,valor:0,lanc:0}); const o=map.get(k); o.valor+=Number(r.saldo||0); o.lanc++;});
