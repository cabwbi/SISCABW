...
function drawPie(id, rows, title){const el=$('#'+id); if(!el)return; if(!window.Plotly){el.innerHTML='<p>Plotly não carregado.</p>';return;} Plotly.newPlot(el,[{type:'pie',labels:rows.map(r=>r.label),values:rows.map(r=>r.value),textinfo:'label+percent',textposition:'auto',automargin:true,hovertemplate:'%{label}<br>'+title+': %{value:$,.2f}<extra></extra>'}],{title:{text:title,font:{size:16}},height:390,margin:{l:155,r:55,t:65,b:40,autoexpand:true}}, {displayModeBar:false,responsive:true});}
