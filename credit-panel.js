...
function drawExecutionPhaseChart(id,rows,limit){
  ...
  const margin=profile.mobile?{l:118,r:18,t:112,b:62}:profile.compact?{l:220,r:125,t:86,b:66}:{l:330,r:190,t:64,b:66};
  Plotly.newPlot(el,[phaseTrace('Crédito não empenhado','available','availablePct','#003b7a'),phaseTrace('Empenhado e não liquidado','committedOpen','committedOpenPct','#2878b8'),phaseTrace('Liquidado e pago','liquidated','liquidatedPct','#73b9e6')],{height:Math.max(profile.mobile?520:470,selected.length*(profile.mobile?39:43)+(profile.mobile?155:105)),barmode:'stack',margin,xaxis:{title:profile.mobile?'% do crédito recebido':'Percentual do crédito recebido',range:[0,100],ticksuffix:'%',fixedrange:true,tickfont:{size:profile.mobile?9:11},titlefont:{size:profile.mobile?10:12}},yaxis:{type:'category',tickmode:'array',tickvals:y,ticktext:y,automargin:true,categoryorder:'array',categoryarray:y,tickfont:{size:profile.mobile?9:12}},legend:{orientation:'h',x:0,y:profile.mobile?1.19:1.09,font:{size:profile.mobile?9:12},traceorder:'normal'},annotations,hovermode:'closest',paper_bgcolor:'rgba(0,0,0,0)',plot_bgcolor:'#fff'}, {displayModeBar:false,responsive:true});
}
...
window.CABW_CREDIT_PANEL_TEST={executionMetrics,executionRows,gcFromDigit,eligibleSignature,rowMatches,filtered,viewportProfile,wrapHoverLabel,omAcronyms,drawExecutionPhaseChart,drawPoBalanceRanking};
