...
function reportBarChart(items){
  ...
  return '<div class="account-bars">'+items.map(i=>'<div class="account-bar-row"><span>'+esc(i.label)+'</span><div class="account-bar-track"><i style="width:'+Math.max(0,Number(i.value||0))/max*100+'%;background:'+esc(i.color||'#0e63b6')+'"></i></div><strong>'+esc(i.display||money(i.value))+'</strong></div>').join('')+'</div>';
}
function contractMonthlyHistoryHtml(r){
  const months=['Jan','Fev','Mar','Abr','Mai','Jun','Jul','Ago','Set','Out','Nov','Dez'];
  const d=monthlyInvoiceForRows([r]); const values=[];
  const tooltip=(label,value,items)=>{const grouped=new Map();(items||[]).forEach(item=>{const invoice=String(item.fatura||'Sem fatura').trim()||'Sem fatura';const po=String(item.po||'').trim();const key=invoice+'|'+po;const row=grouped.get(key)||{invoice,po,value:0};row.value+=Number(item.valor||0);grouped.set(key,row);});const lines=[label+' — '+money(value)];if(!grouped.size)lines.push('Sem faturas no mês');else Array.from(grouped.values()).sort((a,b)=>a.invoice.localeCompare(b.invoice,'pt-BR')).forEach(item=>lines.push(item.invoice+(item.po?' · PO '+item.po:'')+': '+money(item.value)));return lines.join('\n');};
  (d.previous||[]).forEach((value,i)=>{const label=months[i]+'/'+String(d.previousYear).slice(-2);values.push({label,value:Number(value||0),color:'#8d96a6',tooltip:tooltip(label,Number(value||0),(d.previousDetails||[])[i])});});
  (d.current||[]).forEach((value,i)=>{const label=months[i]+'/'+String(d.currentYear).slice(-2);values.push({label,value:Number(value||0),color:'#0e63b6',tooltip:tooltip(label,Number(value||0),(d.currentDetails||[])[i])});});
  const max=Math.max(1,...values.map(i=>i.value));
  return '<div class="month-bars" aria-label="Histórico mensal: cinza para o ano anterior e azul para o ano atual">'+values.map(i=>{const height=Math.max(i.value>0?4:0,i.value/max*82);return '<div class="month-bar" title="'+esc(i.tooltip)+'"><div class="month-bar-track" style="--bar-height:'+height+'%"><span class="month-value">'+chartMoney(i.value)+'</span><i style="height:var(--bar-height);background:'+i.color+'"></i></div><strong>'+esc(i.label)+'</strong></div>';}).join('')+'</div><div class="month-legend"><span><i class="previous"></i>'+esc(String(d.previousYear))+'</span><span><i class="current"></i>'+esc(String(d.currentYear))+'</span></div>';
}
function closedMonthsAverage(r){
  const monthly=monthlyInvoiceForRows([r]);
...
function contractLiquidationTableHtml(r,options){
  const settings=options||{}; const collapsedMonths=Math.max(0,Number(settings.collapsedMonths||0));
  const key=String(r.contrato||'').trim(); const pos=contractPurchaseOrders(r); const byPo=new Map(pos.map(p=>[String(p.po||'').trim(),p]));
  const historical=root.liquidationDetailsByContract&&root.liquidationDetailsByContract[key]; const invoices=Array.isArray(historical)?historical.slice():[];
  ...
  const average=closedMonthsAverage(r); const monthCount=new Set(entries.map(entry=>String(entry.groupDate||'').slice(0,7)||'sem-data')).size;
...
function accountabilityReportHtml(rows,title,updated){
  ...
  const controls='<div class="report-controls"><button id="slidePrev" type="button" onclick="accountabilityPrev()">Página anterior</button><span class="slide-position" id="slidePosition"></span><button class="primary" id="slideNext" type="button" onclick="accountabilityNext()">Próxima página</button><select id="contractJump" onchange="accountabilityJump(this.value)" aria-label="Ir para contrato">'+options+'</select><button type="button" onclick="window.print()">Imprimir / salvar PDF</button></div>';
...
window.CABW_CONTRACTS_PANEL_TEST={contractPurchaseOrders,contractPoBalance,contractMonthlyHistoryHtml,contractLiquidationTableHtml,closedMonthsAverage,accountabilityOmInfo,accountabilityOmOrder,accountabilityContractHtml,accountabilityStyles,accountabilityRowsForCategory,sortAccountabilityRows,administrativeAccountabilityOrder,accountabilityNavigationScript,accountabilityReportHtml,chartMoney};
