...
async function generateContractsReport(){
  const cat=document.body.getAttribute('data-contract-category'); const base=catRows(cat); const f=filters(); const rows=base.filter(r=>rowMatches(r,f,cat)); const a=agg(rows); const chart=await getContractChartImage();
