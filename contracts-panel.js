...
function monthlyChartVariant(source,omCode){
...
function contractMonthlyHistoryHtml(r){
  const source=monthlyInvoiceForRows([r]); const oms=monthlyOmOptions(source); const options=[{code:'ALL',label:'Todas OM',full:'Todas as organizações militares'},...oms];
  ...
  const views=options.map((option,index)=>'<div class="month-history-view" data-om-view="'+esc(option.code)+'"'+(index===0?'':' hidden')+'>'+monthlyChartVariant(source,option.code)+'</div>').join('');
